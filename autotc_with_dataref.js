...
var stepno, stepdescription, expected, Hop, screenname, imgname, fieldname, fieldtype, keyword, selectorval, selectorvalue, mobileselectorvalue, name, retdata, uuid;
var testdata, brdreference, application, automatable, errdetails, status, exedatetime, ngmodel, tagname, reactid, mainframename, iframestatus, dateinmacfor, argoid, noimage,retrievedvalue;
var AngularProperty = "";
var d;
...
var dgref_details = scriptdetails.dgref_details;
////console.log(userdetails , "userdetails");
var crypto = require('crypto');
var instancename = scriptdetails.tindetails.name;
////console.log("instancename", instancename)

var dateinmac = new Date().toLocaleString();

global.lang = true;
global.status = "";
global.msg = "";
global.dgrefvalue="";
dateinmac = dateinmac.replace(/ /g, "_")
dateinmac = dateinmac.replace(/:/g, "_")
dateinmac = dateinmac.replace(/-/g, "_")
//console.log(dateinmac, "Date above test")
var scrfilename = instancename + '_' + userdetails + '_' + dateinmac

console.log(scrfilename)
try {
    fs.mkdir('./mypic/' + scrfilename, function (e) {
...
//var io = require('socket.io-client') //creating a client socket to connect to aqua server
//var socket = io.connect('http://localhost:'+sockno);
    ...
    .after(() => {
        ...
        if (global.cflag == scriptdetails.scriptstep.length - 1) {
            ...
            var tempobj = {
                ...
                "dgref_details": scriptdetails.dgref_details,
...
async function waituntil() {
    return new Promise(async function (resolve, reject) {
        var timetowait = testdata * 1000; // testdata in seconds
        //  console.log( "start of delay", testdata)
        await setTimeout(function () {
            global.status = "Pass"
            global.msg = "The step has waited till  " + testdata + "  seconds successfully";
            //console.log(   global.msg)
            return resolve();
        }, timetowait);
        //  console.log( "End of delay", timetowait )
    });
}
...
function sendscreenshot(status, errdetails, exedatetime, stepdata, imgname) {

    // console.log("sending screenshot to tram......")
    async.auto({



        getImageData: async function (callback) {
            if ((stepdata.keyword == 'sql') || (stepdata.keyword == 'email') || (stepdata.keyword == 'pdf') || (stepdata.keyword == 'waituntil') || (stepdata.type == 'file') || (stepdata.step_skip)) {

            } else {


                var bitmap = './mypic/' + imgname


                var temp_d = await fs.readFileSync(bitmap) //, async function(err, res) {

                var image = await temp_d.toString('base64')

                //if(res){
                //console.log("base64 imgae string")

                // console.log(image.length,"746  screenshot")    
                callback(null, image);
        ...
        write_file: ['getImageData', function (results, callback) {
            ...
            tempobj.scriptstep = []
            //cur_object.screenshot =  correctimg 
            // console.log(cur_object.screenshot.length,"Res",temp_d.toString('base64').length,"746  screenshot")    
            //tempobj.scriptstep.push(stepdata)
            tempobj.currobj = stepdata
            tempobj.type = 'screenshot'
...
function sendtramdata(status, errdetails, exedatetime, stepdata, imgname,retrievedvalue) {
    ...
    stepdata.status = status;
    stepdata.actual = errdetails;
    stepdata.dgrefvalue = retrievedvalue
    console.log(stepdata.dgrefvalue,"stepdata.dgrefvalueeeeee")
    //tempobj.scriptstep.push(stepdata)
    tempobj.currobj = stepdata
    tempobj.type = 'status'
    // socket.emit('auto-exeres', tempobj)
...
test('Rabo Automation Test', async t => {
    ...
    const pageLoadCheck = ClientFunction(function () {
        ...
        var interval = setInterval(function () {
            ...
            if (document.readyState === 'complete' || LoadCheckcounter == 100) {
    ...
    /* var folderwatcher = require('folder-watcher');
  
    folderwatcher.on('./mypic/'+scrfilename,async (object) => {
    // console.log('File modified: %s',object.event, object.file);
     //i++;
     try{
     if(object.event == 'create'){
         console.log("created")
         var splitdata=object.file.split('/')
         var imgn=splitdata[splitdata.length-1]
         var ind=imgn.substring(0,imgn.indexOf('.'))
 
         console.log(splitdata,imgn,imgn.substring(0,imgn.indexOf('.')))
          console.log('File modified: %s',object.event, object.file  ,"725",object);
          var stepargoid = stepdata.filter(function(id){
            return id.argoid == ind;
          });

          
          
         var cur_object = stepargoid[0]
         var imagepath = await cur_object.imgname 
         var temp_d = await fs.readFileSync('./mypic/' + scrfilename+"/"+ind+".png")//, async function(err, res) {
                       console.log(temp_d, "Response")
         //if(res){
                 console.log("base64 imgae string")
         ...
         tempobj.scriptstep = []        
         cur_object.screenshot =  await temp_d.toString('base64');   
         console.log(cur_object.screenshot.length,"Res",temp_d.toString('base64').length,"746  screenshot")    
         //tempobj.scriptstep.push(stepdata)
         tempobj.currobj = cur_object
         tempobj.type='screenshot'
         // socket.emit('auto-exeres', tempobj)
      //   console.log("tempobj", "-------------------------",tempobj)
         var auto_json = JSON.stringify(tempobj)
         console.log("dealer sent")
         await dealer.send(auto_json)
         tempobj = {}   
         cur_object.screenshot = null  
    ...
    for (var i = 0; i < stepdata.length; i++) {
        ...
        dateinmacfor = new Date().toString();
       ...
       if (stepdata[i].dgref_id){
            var tddata_verify = stepdata[i].dgref_id

            var tddata_data_ref = stepdata[i].testdata_ref

                var testdata_set = tddata_verify.split("," )


                console.log('????????********//////?',tddata_data_ref,testdata_set.length)


                if (typeof(testdata_set)=='undefined'){

            /*        var tdrefassign_init = tddata_verify

                    var tdrefassign = tddata_verify
                    
                    tdrefassign = tdrefassign.split(".");
                    tdrefassign = tdrefassign[0]+"."+tdrefassign[1]
                    var trfefreplace = "{{" +tdrefassign +  "}}" 

                    var testdata_temp= dgref_details[tdrefassign_init]


                    tddata_data_ref= tddata_data_ref.replace(trfefreplace,testdata_temp)
                    

                    console.log('8899999999---------',tdrefassign_init,'***********',tddata_data_ref,testdata_temp,'+++++',trfefreplace,'-----',dgref_details,'/////',tdrefassign)

                    testdata = tddata_data_ref;*/

                } else {

                    tddata_verify.split("," ).forEach(function (testdata_set_val) {
                    /*    document.getElementById(item).style.visibility = "visible";
                    });

             for (var tdrefloop = 0 ; tdrefloop < testdata_set.length;tdrefloop++ ){*/

                    //console.log('??????*******???',testdata_set,testdata_set_val)
                    var tdrefassign_init = testdata_set_val
                    var tdrefassign = testdata_set_val
                    tdrefassign = tdrefassign.split(".");
                    tdrefassign = tdrefassign[0]+"."+tdrefassign[1]
                    var trfefreplace = "{{" +tdrefassign +  "}}" 
                    var testdata_temp= dgref_details[tdrefassign_init]
              ...
              //  console.log('889---------',tdrefassign_init,'***********',tddata_data_ref,testdata_temp,'+++++',trfefreplace,'-----',dgref_details,'/////',tdrefassign)
                })
                 //console.log('inside dgreference ------------------------------------------------')
                testdata = tddata_data_ref;
            } 
  ...
  testdata = stepdata[i].testdata;
        ...
        argoid = stepdata[i].argoid;
        retdata = stepdata[i].variable;
        var testcasename = stepdata[i].name;
        //var dgrefvalue = stepdata[i].dgrefvalue
        ...
        // check dgreference and pull required data 
        ...
        if ((testdata.indexOf("{") != -1) && (testdata.indexOf("{{") == -1)) {
        ...
        if (step_skip) {
            ...
            if ((fieldtype == "url") || (keyword == "email") || (keyword == "pdf") || (keyword == "waituntil") || (fieldtype == "treestructure") ||  (fieldtype == "file") || (fieldtype == "windowbutton")) {
                                ...
                                try {
                                    await t.click(Selector(selectorval))
                                        ...
                                        await setValue.with({
                                            ...
                                            global.eventStatus = "success";
                                            global.status = "Pass"
                                            global.msg = "The '" + testdata + "' was successfully entered in the textbox '" + fieldname + "' in the '" + screenname + "' page";

                                        }, function (failmsg) {
                                            global.status = "Fail"
                                            global.msg = "Unable to enter the '" + testdata + "' in the textbox '" + fieldname + "' in the '" + screenname + "' page. SYSTEM ERROR: " + failmsg;
                                            global.eventStatus = "fail";
                                        });
                                    ...
                                    // //console.log("Error:" + err)
                                    global.status = "Fail"
                                    global.msg = "Unable to enter the '" + testdata + "' in the textbox '" + fieldname + "' in the '" + screenname + "' page. SYSTEM ERROR: " + err;
                                    global.eventStatus = "fail";
                                }
...
*/
                                ...
                                if (allcolverify.indexOf("#all#") != -1) {
                                    ...
                                    if (countoftablecellobjs > 0) {
                                        for (var tdcount = 0; tdcount < countoftablecellobjs; tdcount++) {
                                            ...
                                            if (screenname == "Account_Statement" && global.lang == true && tabletestdata[0].indexOf("/") != -1) {
                                                tabletestdata[0] = moment(tabletestdata[0], ["DD/MM/YYYY", "YYYY/MM/DD"]).format('DD[/]MM[/]YYYY')
                                            }
                                            ...
                                            if (tddata == tabletestdata[0]) {
                                                ...
                                                if (sibdatacount == 1) {
                                                    ...
                                                    if (screenname == "Account_Statement" && global.lang == true && tabletestdata[1].indexOf("/") != -1) {
                                                        tabletestdata[1] = moment(tabletestdata[1], ["DD/MM/YYYY", "YYYY/MM/DD"]).format('DD[/]MM[/]YYYY')
                                                    }
                                                            ...
                                                            if ((elsetableflag) && (listdatacount > 2)) {
                                                                ...
                                                                for (var datacount = 2; datacount < listdatacount; datacount++) {
                                                                    for (var txt = 0; txt < lastcount; txt++) {
                                                                        ...
                                                                        if (screenname == "Account_Statement" && global.lang == true && tabletestdata[datacount].indexOf("/") != -1) {
                                                                            tabletestdata[datacount] = moment(tabletestdata[datacount], ["DD/MM/YYYY", "YYYY/MM/DD"]).format('DD[/]MM[/]YYYY')
                                                                        }
                        ...
                        case "text":
                            ...
                            case "file":
                                try {
                                    ...
                                    // console.log("systimenow", systimenow, timeoflastfile, "timeoflastfile" );
                ...
                case "waituntil":
                    try {
                        ...
                        global.status = "Pass"
                        global.msg = "The script has waited till  " + testdata + "  seconds successfully";
                    } catch (err) {
                        global.status = "Fail"
                        global.msg = "The script failed to waited till '" + testdata + "  seconds";
                ...
                case "pdf":
                    ...
                    try {
                        ...
                        typeofdownfile = typeofdownfile.toLowerCase();
                        //  selectorval = selectorval.toLowerCase();
                        selectorval = typeofdownfile
                        await t.wait(1500)
                        // var TESTFILE = typeofdownfile
                        if (timediff <= 2) {
                            var TESTFILE = typeofdownfile
                            // }
                            // } else {

                            /*   console.log("selctor not empty")
                            var TESTFILE = selectorval;
                           // console.log(TESTFILE, "2345")
                            ...
                            function printRawItems(callback) {
                                ...
                                parseFileItems(TESTFILE, function (err, item) {
                                    if (err) {
                                        ...
                                        callback();
                                    else { //console.log(item, "item is printed");  
                                    }
                            ...
                            function forEachItem(pdf, handler) {
                                ...
                                for (var p in pdf.data.Pages) {
                                    ...
                                    if (jtext.toLowerCase().match(testdata.toLowerCase()) == null) {
                                        ...
                                        global.status = "Pass";
                                        global.msg = "The content  '" + pdfvalue + "' successfully verified in the PDF";
                                        //  //console.log("Test Data Found")
                                        found();
                                    }
                                    //break;
                            ...
                            global.msg = "Unable to find the PDF, Please Check whether the PDF is downloaded recently in the correct location";

                        }
                    } catch (err) {
                        global.status = "Fail";

                        global.msg = "Unable to find the PDF, Please Check whether PDF is available in the correct location";
                        ...
                        if (textval == true) {
                          ...
                          /*   for (var reti = i; reti < stepdata.length; reti++) {
                            ...
                            if (stepdata[i].dgref_id){
                                ...
                                var tddata_ret_array = stepdata[i].dgref_id;

                                dgref_details[tddata_ret_array] = retdata

                               // console.log(retdata,"retdata")
                              //  console.log(dgref_details, "dgref_detailssssssssss")

                             //   console.log(retdata ,"value retureddddddd")
                                
                                global.dgrefvalue =  String(retdata) 
                             //   console.log(global.dgrefvalue ,"dgrefvalue retureddddddd")

                        /*        var tddata_verify = stepdata[i].dgref_id;
                                var tddata_data_ref = stepdata[i].testdata_ref
                                    var testdata_set = tddata_verify.split("," )
                                    for (var tdrefloop = 0 ; tdrefloop <=testdata_set.length;tdrefloop++ ){
                                        var tdrefassign = testdata_set[i]
                                        var trfefreplace = "{{" +tdrefassign + "}}"
                                        var testdata_temp= dgref_details[tdrefassign]
                                        tddata_data_ref.replace(trfefreplace,testdata_temp)
            ...
            status = global.status;
            errdetails = global.msg;
            retrievedvalue = global.dgrefvalue;
            exedatetime = new Date();
            sendtramdata(status, errdetails, exedatetime, stepdata[i], imgname,retrievedvalue);
            await pageLoadCheck();
            ...
            if ((keyword == 'pdf') || (keyword == 'email') ||  (fieldtype == 'file')) {
                ...
                var oo = await t.takeScreenshot(scrfilename + "/" + argoid + ".png").then(async function (res) {
                    //console.log("inside Take Screenshot");
                    var imgname = scrfilename + "/" + argoid + ".png"
                    // console.log('4156',res)
                    await sendscreenshot(status, errdetails, exedatetime, stepdata[i], imgname);
                });
            }
